// CartRadar — Aldi Content Script
// Aldi uses aldi.us with a store locator and product search
// Products loaded via their internal API at aldi.us/api
...
(function() {
  ...
  async function extractProducts(query) {
    // Aldi loads products via XHR to their internal API
    // Try API first, then DOM fallback
    try {
    ...
    // DOM extraction
    await waitForSelector('[data-testid="product-tile"], .product-tile, [class*="product"]', 5000);

    const items = [];
    const cards = document.querySelectorAll(
      '[data-testid="product-tile"], .product-tile, [class*="ProductCard"], [class*="product-card"]'
    );
  ...
  async function extractFromAPI(query) {
    // Aldi's product search API — no auth needed from their own domain
    const resp = await fetch(
      `https://www.aldi.us/api/products/search?q=${encodeURIComponent(query)}&limit=15`,
      {
        headers: {
    ...
    return products.map(p => ({
      ...
      url: p.url || p.productUrl || `https://www.aldi.us/product/${p.id || p.sku || ''}`,
      unitPrice: calcUnitPrice(
        parseFloat(p.price || p.currentPrice || 0),
        p.size || p.packageSize || ''
      ),
  ...
  function extractCard(card) {
    ...
    const sizeEl = card.querySelector('[class*="size"], [class*="weight"], [class*="package"]');
