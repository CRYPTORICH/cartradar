// CartRadar — H-E-B Content Script
// H-E-B is the dominant Texas grocer (300+ stores, $38B revenue)
// heb.com has full online shopping with search
...
(function() {
  ...
  async function extractProducts(query) {
    ...
    // DOM fallback
    await waitForSelector('[class*="product"], .product-card, [class*="search-result"]', 5000);

    const items = [];
    const cards = document.querySelectorAll(
      '[class*="product-card"], [class*="ProductCard"], [class*="product-tile"], .product-item'
    );
  ...
  async function extractFromAPI(query) {
    // H-E-B product search API
    const resp = await fetch(
      `https://www.heb.com/api/search?q=${encodeURIComponent(query)}&limit=15`,
      {
    ...
    return products.map(p => ({
      name: cleanName(p.name || p.title || p.productName || p.description || ''),
      price: parseFloat(p.price || p.currentPrice || p.listPrice || 0),
      size: p.size || p.packageSize || p.description || '',
      image: p.image || p.imageUrl || p.thumbnail || '',
      url: p.url || `https://www.heb.com/product/${p.id || p.sku || ''}`,
